...
def page2(c, d):
    ...
    p = Paragraph(d["avis_valeur"], style_avis)
    ...
    rows = [
        ("Valeur minimale",   fmt(d["prix_min"]),        False),
        ("Valeur maximale",   fmt(d["prix_max"]),        False),
        ("Valeur retenue",    fmt(d["prix_retenu"]),      True),
        ("Prix sans décote",  fmt(d["prix_sans_decote"]), False),
        ("Prix avec décote",  fmt(d["prix_avec_decote"]), False),
    ]
    ...
    signataires = [
        (d["negociateur"],  "Négociateur mandataire"),
        ("Laurent Baradu",  "Directeur — Barbier Immobilier"),
    ]

    for i, (name, role) in enumerate(signataires):
        sx = ML + i * (sig_w + 14)
...
def footer(c, page_n):
    ...
    c.saveState()
    c.setFillColor(GRAY_MID); c.setFont("Helvetica", 6.5)
    c.drawString(ML, y - 1, "DOCUMENT CONFIDENTIEL  ·  Barbier Immobilier  ·  Vannes")
...
def generate(out_path, data=None):
    if data is None:
        raise ValueError("data required")
    c = rl_canvas.Canvas(out_path, pagesize=A4)
    c.setTitle("Avis de Valeur — Barbier Immobilier")
    c.setAuthor("Barbier Immobilier · 9•58 Consulting")

    page1(c, data); footer(c, 1); c.showPage()
    page2(c, data); footer(c, 2); c.showPage()
...
def seatable_get_row(reference):
    """Récupère une ligne SeaTable par référence, retourne un dict avec noms lisibles."""
    ...
    # Requête SQL — récupérer tous les champs utiles
    sql = f"""
    SELECT `Reference`, `Type de bien`, `Surface`, `Surface terrain`,
           `Adresse`, `Ville`, `Code postal`, `Etat du bien`,
           `Negociateur`, `Nom client`, `Référence cadastrale`,
           `Prix estime min`, `Prix estime max`, `Prix retenu`,
           `Prix sans décote`, `Prix avec décote`,
           `Avis de valeur`
    FROM `01_Biens`
...
def seatable_update_statut(reference, statut):
    ...
    resp = requests.post(
        ...
        json={"sql": f"SELECT _id FROM `01_Biens` WHERE `Reference` = '{reference}' LIMIT 1",
              "convert_keys": False}, timeout=10
    )
    results = resp.json().get("results", [])
    if not results:
        return
    row_id = results[0].get("_id")

    requests.put(
        f"https://cloud.seatable.io/api-gateway/api/v2/dtables/{UUID}/rows/",
        headers={"Authorization": f"Token {AT}", "Content-Type": "application/json"},
        json={"table_name": "01_Biens", "updates": [
            {"row_id": row_id, "row": {"Statut avis valeur": statut}}
        ]}, timeout=10
    )
...
def get_osm_map(address, out_w=840, out_h=340, zoom=16):
    try:
        ...
        app.logger.error(f"OSM error: {e}")
        return None
...
def draw_logo(c, x, y, logo_buf=None, size=36):
    """Dessine le logo depuis buffer image ou fallback vectoriel."""
    if logo_buf:
        try:
            logo_buf.seek(0)
            c.drawImage(rl_canvas.ImageReader(logo_buf), x, y - size,
                        width=size * (488/662), height=size, mask='auto',
    ...
    c.drawCentredString(x + size/2, y - size * 0.65, "B")
    c.restoreState()
...
def page1(c, d, logo_buf=None):
    ...
    # Header
    draw_logo(c, ML, y, logo_buf, size=36)
    ...
    c.setFillColor(GRAY_MID)
    c.setFont("Helvetica", 8)
    nom_client = d.get("Nom client") or ""
    c.drawRightString(PAGE_W - MR, y - 25,
                      f"Réf. {d.get('Reference', '')}  ·  {d.get('Ville', '')}  ·  {nom_client}")
    c.restoreState()
    ...
    surf_terrain = d.get("Surface terrain")
    fields = [
        ("Type de bien",      d.get("Type de bien") or "—"),
        ("Surface habitable", f"{d.get('Surface', '—')} m\u00b2"),
        ("Surface terrain",   f"{surf_terrain} m\u00b2" if surf_terrain else "—"),
        ("Adresse",           d.get("Adresse") or "—"),
        ("Réf. cadastrale",   d.get("Référence cadastrale") or "—"),
        ("État général",      d.get("Etat du bien") or "—"),
    ]
    ...
    for i, (lbl, val) in enumerate(fields):
        ...
        val_display = val[:38] + "…" if len(str(val)) > 38 else val
        c.drawString(cx+6, cy - 20, str(val_display))
        c.restoreState()
    ...
    map_h = 88*mm
    adresse_full = f"{d.get('Adresse', '')}, {d.get('Code postal', '')} {d.get('Ville', '')}"
    map_buf = get_osm_map(adresse_full, out_w=840, out_h=340, zoom=16)
    ...
    c.saveState()
    c.setFillColor(TEAL); c.setFont("Helvetica-Bold", 7.5)
    c.drawString(ML, y, f"\u25a0  {adresse_full}")
    c.restoreState()
    y -= 14
    ...
    prix_min    = d.get("Prix estime min")
    prix_retenu = d.get("Prix retenu")
    prix_max    = d.get("Prix estime max")
    ...
    cards = [
        ("VALEUR BASSE",   prix_min,    False),
        ("VALEUR RETENUE", prix_retenu,  True),
        ("VALEUR HAUTE",   prix_max,    False),
    ]
    ...
    for i, (lbl, prix, rec) in enumerate(cards):
        ...
        label_y = cy + card_h - (30 if rec else 14)
        c.saveState()
        c.setFillColor(tc); c.setFont("Helvetica", 6.5)
        c.drawCentredString(cx + card_w/2, label_y, lbl)
        c.restoreState()
        ...
        prix_str = fmt(prix)
        prix_y = cy + card_h/2 - (19 if rec else 18)
        c.saveState()
        c.setFillColor(pc); c.setFont("Helvetica-Bold", 15 if rec else 13)
        c.drawCentredString(cx + card_w/2, prix_y, prix_str)
        c.restoreState()
    ...
    c.drawString(ML + 10, y - b_h/2 - 3, "PRIX DE MISE EN MARCH\u00c9 CONSEILL\u00c9")
    c.setFillColor(GRAY_DARK); c.setFont("Helvetica-Bold", 12)
    c.drawRightString(PAGE_W - MR - 10, y - b_h/2 - 4, fmt(prix_retenu))
    c.restoreState()

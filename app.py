...
def _draw_atouts_cards(c, d, x, y, total_w):
    ...
    # Mise en page : 4 cartes côte à côte
    n = min(len(atouts), 4)
    gap = 3 * _mm
    card_w = (total_w - (n - 1) * gap) / n
    card_h = 42 * _mm
    for i, atout in enumerate(atouts[:n]):
        cx = x + i * (card_w + gap)
        cy = y - card_h
        # Fond bleu foncé
        ...
        c.setFillColor(_ORANGE)
        c.roundRect(cx, cy + card_h - 3 * _mm, card_w, 3 * _mm, 2 * _mm, fill=1, stroke=0)
        # Titre
        titre = atout.get("titre", "").upper()
        c.setFillColor(_ORANGE)
        c.setFont("Helvetica-Bold", 7.5)
        c.drawString(cx + 4 * _mm, cy + card_h - 9 * _mm, titre[:22])
        # Texte
        texte = atout.get("texte", "")
        para = _Para(texte, _PS("ac", fontName="Helvetica", fontSize=7.5,
                                textColor=_BLANC, leading=11, alignment=4))
        tw = card_w - 8 * _mm
        _, ph = para.wrap(tw, 9999)
        para.drawOn(c, cx + 4 * _mm, cy + card_h - 13 * _mm - ph)
